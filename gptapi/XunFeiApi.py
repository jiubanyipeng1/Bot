    ...
    try:
        if stream:
            ...
            if response.status_code != 200:  # 这里失败的一般是配置有问题
                return {'code': False, "data": response.json(), 'info': '讯飞的配置接口错误，请检查配置文件！'}
            ...
            if response.json().get('code', 99) == 0:
                ...
                return {'code': False, "data": response.json(), 'info': '讯飞接口错误返回错误，请求速度太快或额度不足等，请查看详情'}
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=body, headers={'Content-Type': 'application/json'}) as response:
                if response.status == 200:
                    ...
                    return {'code': True, "data": data["payload"]["choices"]["text"][0]['content'], 'sid': data['header']['sid']+'.jpg', 'info': '讯飞 文本生成图片 返回成功'}
                else:
