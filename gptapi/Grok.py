import requests
import json
def generate_text(text, config_api) -> dict:
    """ 生成文本
    ...
    """
    url = config_api.get('url', 'https://api.x.ai/v1/chat/completions')
    model = config_api.get('model', 'grok-2-1212')
    ...
    try:
        if stream:
            ...
            if response.status_code != 200:  # 这里失败的一般是配置有问题
                return {'code': False, "data": response.text, 'info': 'Grok的配置接口错误，请检查配置文件！'}

            return {'code': True, "data": stream_response(), 'info': 'Grok聊天接口返回'}
        else:
            response = requests.post(url, headers=header, json=parameters,proxies=proxies)
            if response.status_code != 200:  # 这里失败的一般是配置有问题
                return {'code': False, "data": response.json(), 'info': 'Grok的配置接口错误，请检查配置文件！'}
            return {'code': True, "data": response.json()['choices'][0]['message']['content'],'info': 'Grok聊天接口返回'}

    except requests.exceptions.RequestException as e:
        return {'code': False, "data": str(e), 'info': '可能是网络问题，或配置文件的url地址不对！'}

    except Exception as e:
        return {'code': False, "data": str(e), 'info': '程序报错！有可能Grok口返回的数据格式不对！'}